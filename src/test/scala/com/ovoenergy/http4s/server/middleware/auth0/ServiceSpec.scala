...
import com.ovoenergy.http4s.Http4sMatchers
...
class ServiceSpec extends WordSpec with Http4sMatchers with OptionValues with BeforeAndAfterAll {
  ...
  "Service" should {
    ...
    "return 401 if no Authorization header is provided" in {
      val service = Service(simpleService, createConfig())
      val request = baseRequest.withHeaders(Headers.empty)
      service.run(request).value.unsafeRunSync() should haveStatus(Status.Unauthorized)
    }

    "return custom status if no Authorization header is provided and a non-standard unauthenticated status was provided" in {
      val customStatus = Status.NetworkAuthenticationRequired
      val service = Service(simpleService, createConfig(customStatus))
      val request = baseRequest.withHeaders(Headers.empty)
      service.run(request).value.unsafeRunSync() should haveStatus(customStatus)
    }

    "return 401 if no Bearer token is present in Authorization header" in {
      val service = Service(simpleService, createConfig())
      val request = baseRequest.withHeaders(Headers(basicTokenHeader("basic-credentials")))
      service.run(request).value.unsafeRunSync() should haveStatus(Status.Unauthorized)
    }

    "return custom status if no Bearer token is present in Authorization header and a non-standard unauthenticated status was provided" in {
      val customStatus = Status.NetworkAuthenticationRequired
      val service = Service(simpleService, createConfig(customStatus))
      val request = baseRequest.withHeaders(Headers(basicTokenHeader("basic-credentials")))
      service.run(request).value.unsafeRunSync() should haveStatus(customStatus)
    ...
    "return 401 if a bad Bearer token is supplied" in {
    ...
    "return custom status if a bad Bearer token is supplied and a non-standard unauthenticated status was provided" in {
      val customStatus = Status.NetworkAuthenticationRequired
      val token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsImtpZCI6IjJ0TEJuLVBtSHgzdXRfRkQ5dmJkSmtLNVUzV1FEMzk2US1nMExpRlg5U0kifQ.eyJpc3MiOiJldnNlIGRldmljZSBpbnRlZ3JhdGlvbiIsIm5iZiI6MTUxMzU0NDkwM30.OUK-Ipty21TIILhLg-ImFhNpfabXCb0BB-_eASjuj-QcVZdOZb42CIZAkSpjGbpScFjcBcM_kZh3OJnxP3uhj9hCRDqfDiu2YpoXitLwcNlvf_I-XfZxGCVFxZSiktcu3TMqL1JIPNWwpZoSIZvo3d0ubXmlxdjfXdGVC7Rk_YWjKWjVpxLOWuPfKplsCtpe07DTcpIkazzr7Nx4eFcrxajZUlBqgxPSznvTJN0jcaegB8_-HhoYyQPosox3haKFKxf1Cg9MslmVq746lTZWlDlq5no3WyIXmLiizWsit1fuSHFZIeWnIYQqsksgKNe0z_6Kv4XEsHlIHeXKgw0FJA"
      val service = Service(simpleService, createConfig(customStatus))
      val request = baseRequest.withHeaders(Headers(bearerTokenHeader(token)))
    ...
    "return 401 if an unsigned Bearer token is supplied" in {
    ...
    "return custom status if an unsigned Bearer token is supplied and a non-standard unauthenticated status was provided" in {
      val customStatus = Status.NetworkAuthenticationRequired
      val token= "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiIsImtpZCI6IjZfWWZvbHh4elJZUDR5cmdFSWlOaXRVNzNpUEVGNnlhQTdVR29NNk5KRXcifQ.eyJpc3MiOiJldnNlIGRldmljZSBpbnRlZ3JhdGlvbiIsIm5iZiI6MTUxMzU5MDU3NX0."
      val service = Service(simpleService, createConfig(customStatus))
      val request = baseRequest.withHeaders(Headers(bearerTokenHeader(token)))
    ...
    "return 401 if no 'kid' present in supplied Bearer token" in {
    ...
    "return custom status if no 'kid' present in supplied Bearer token and a non-standard unauthenticated status was provided" in {
      val customStatus = Status.NetworkAuthenticationRequired
      val token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJSUzI1NiJ9.eyJpc3MiOiJldnNlIGRldmljZSBpbnRlZ3JhdGlvbiIsIm5iZiI6MTUxMzU5MDgxMH0.X4aacxq-E-SMyUGjXfbfPG0UZdYXXj6t_1Y47-8C6SPaQcsKE5g8YYMoaASu5KQU1php_BMKEwkiC1zVukEviCOcWw3ihhM9l9gogK99EdJF0BYTk8kkRXoM42QIED3ORScod-JcHGv2_yYVucaUMC1Fw-YAekyhxyMwBahrBsNAyDRCvZeE4yO9QrlFEb_KZkAaxYCuL_2o9ObWJ3EhpJSWFgeZUzD1uZ7SEPR9677HHPmaO8XCy_W76XjE5O17uacquFlOYAORkQAqqIYnVIlDyO5qw58Mk1vfAOskycOd0hLLzTjwItsZmQhqSOCrgOjz69XTWw2qrVvm0iGRQA"
      val service = Service(simpleService, createConfig(customStatus))
      val request = baseRequest.withHeaders(Headers(bearerTokenHeader(token)))
  ...
  private lazy val jwksDomain: String = s"http://$host:${wireMockServer.port()}/"

  private val jwksPath: String = "/.well-known/jwks.json"

  private def bearerTokenHeader(token: String): Header = Authorization(Token(AuthScheme.Bearer, token))

  private def basicTokenHeader(token: String): Header = Authorization(Token(AuthScheme.Basic, token))

  private def createConfig(customStatus: Status = Status.Unauthorized): Config =
    Config(new UrlJwkProvider(jwksDomain), customStatus)
